...
function dns_info_dmarc_record_check() {
	$result = array(
		'label'       => __( 'DMARC Record is properly configured' ),
		'status'      => 'good',
		'badge'       => array(
			'label' => __( 'Security' ),
			'color' => 'blue',
		...
		'description' => sprintf(
			'<p>%s</p>',
			__( 'Your DMARC record is properly configured, helping to prevent email spoofing.' )
		),
		'actions'     => '',
		'test'        => 'dmarc_record',
	);
	...
	if ( empty( $dmarc_record ) ) {
		$result['status']      = 'recommended';
		$result['label']       = __( 'DMARC Record is not found' );
		$result['description'] = sprintf(
			'<p>%s</p>',
			__( 'Your DMARC record is not found. It is recommended to create and configure an DMARC record to prevent email spoofing.' )
		);
		$result['actions']    .= sprintf(
			'<p>%s</p>',
			__( 'Please consult your DNS provider or system administrator to create an DMARC record for your domain.' )
		);
	}
